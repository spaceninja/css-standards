...
Welcome to the Space Ninja CSS standards. This document outlines our best practices and philosophies when it comes to writing CSS. We follow three guiding principles to produce code that is modular and easy to maintain.
...
Before you write CSS for any product following these standards, please take the time to read this document carefully. If you haven’t already, you should also check out Jonathan Snook’s [SMACSS](https://smacss.com/) book. It’s a quick read, and the bulk of our philosophy is based on his approach.
...
These formatting rules should be reflected in a `.stylelintrc` file in the root of your project. (Our standard `.stylelintrc` file can be found in [stylelint-config-spaceninja](https://github.com/spaceninja/stylelint-config-spaceninja).) This file can be read by your text editor, or run as part of the existing project lint tests to enforce our house style.
...
* Avoid writing `TODO` and `FIXME` comments. CSS is not an issue tracker. Instead, file a ticket with the change that needs to be made.
...
* **Vendor prefixing**: We use [Autoprefixer](https://github.com/postcss/autoprefixer) to add prefixes when needed and keep our code clean.

* **Reset:** We favor [Normalize.css](https://necolas.github.io/normalize.css/).
